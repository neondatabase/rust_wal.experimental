...
use async_raft::raft::{Entry, EntryConfigChange, EntryPayload, MembershipConfig};
use async_raft::storage::{CurrentSnapshotData, HardState, InitialState};
pub use async_raft::{AppData, AppDataResponse, NodeId, RaftStorage};
...
async fn test_get_membership_config_default() -> Result<()> {
    let store = WALStore::new(NODE_ID);
...
async fn test_get_membership_config_with_previous_state() -> Result<()> {
    ...
    let sm = WALStoreStateMachine::default();
    let hs = HardState {
        current_term: 1,
        voted_for: Some(NODE_ID),
    };
    let store = WALStore::new_with_state(NODE_ID, log, sm, Some(hs.clone()), None);
...
fn default_store_with_logs() -> WALStore {
    let mut log = BTreeMap::new();
    log.insert(
        1,
        Entry {
    ...
    WALStore::new_with_state(NODE_ID, log, sm, Some(hs), None)
}
